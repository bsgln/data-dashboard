...
const LIVE_API_URL = "https://e-mongolia.mn/shared-service/api/survey/stats/v22/688b1679eeaf8f6a1fab1011";
...
async function fetchLiveQuestions(): Promise<SurveyQuestion[]> {
  try {
    ...
    for (const [questionText, responses] of Object.entries(questionsData)) {
      ...
      const totalVotes = responses.reduce((sum: number, item: any) => sum + (item.count || 0), 0);
      
      // Create response objects
      const questionResponses: SurveyResponse[] = responses.map((item: any, index: number) => {
